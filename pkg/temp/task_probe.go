...
func (TaskProbe *TaskProbe) GetCommoditiesSoldByContainer(task *util.Task, taskResourceStat *TaskResourceStat) []*proto.CommodityDTO {
	var commoditiesSold []*proto.CommodityDTO
	memAllocationComm, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_MEM_ALLOCATION).
						Key(task.Id).
	...
	cpuAllocationComm, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_CPU_ALLOCATION).
							Key(task.Id).
	...
	diskAllocationComm, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_STORAGE_ALLOCATION).
							Key(task.Id).
...
// Build commodityDTOs for commodity sold by the pod
func (taskProbe *TaskProbe) GetCommoditiesBoughtByContainer(task *util.Task, taskResourceStat *TaskResourceStat) []*proto.CommodityDTO {
	var commoditiesBought []*proto.CommodityDTO
	cpuAllocationCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_CPU_ALLOCATION).
							Key("Mesos").
							Used(taskResourceStat.cpuAllocationUsed).
							Create()
	commoditiesBought = append(commoditiesBought, cpuAllocationCommBought)
	memAllocationCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_MEM_ALLOCATION).
							Key("Mesos").
							Used(taskResourceStat.memAllocationUsed).
							Create()
	commoditiesBought = append(commoditiesBought, memAllocationCommBought)
	diskAllocationCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_STORAGE_ALLOCATION).
							Key("Mesos").
							Used(taskResourceStat.diskAllocationUsed).
							Create()
...
func (TaskProbe *TaskProbe) GetCommoditiesSoldByApp(task *util.Task, taskResourceStat *TaskResourceStat) []*proto.CommodityDTO {
	appName := task.Name
	var commoditiesSold []*proto.CommodityDTO
...
func (taskProbe *TaskProbe) GetCommoditiesBoughtByApp(task *util.Task, taskResourceStat *TaskResourceStat) map[*builder.ProviderDTO][]*proto.CommodityDTO {
	...
	slaveProvider := builder.CreateProvider(proto.EntityDTO_VIRTUAL_MACHINE, task.SlaveId)
	var commoditiesBoughtFromSlave []*proto.CommodityDTO

	vCpuCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_VCPU).
					//		Key(task.SlaveId).
					Used(taskResourceStat.cpuAllocationUsed).
					Create()
	commoditiesBoughtFromSlave = append(commoditiesBoughtFromSlave, vCpuCommBought)

	vMemCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_VMEM).
					//		Key(task.SlaveId).
					Used(taskResourceStat.memAllocationUsed).
					Create()
	commoditiesBoughtFromSlave = append(commoditiesBoughtFromSlave, vMemCommBought)

	appCommBought, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_APPLICATION).
					Key(task.SlaveId).
					Create()
