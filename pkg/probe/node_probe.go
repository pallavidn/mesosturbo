...
func (nodeProbe *NodeProbe) CreateCommoditySold(slaveInfo *util.Slave, useMap map[string]*util.CalculatedUse) ([]*proto.CommodityDTO, error) {
	...
	*/
	...
	appComm, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_APPLICATION).
		Key(slaveInfo.Id).
		Create()
	...
	for _, p := range nodeProbe.AllSlavePorts {
		...
		if p != "" {
			if portObj, ok := ports[p]; ok {
				portComm, _ := builder.NewCommodityDTOBuilder(proto.CommodityDTO_NETWORK).
