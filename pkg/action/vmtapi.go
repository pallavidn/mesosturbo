...
func (r *Reservation) GetVMTReservation(uuidString string, pending []*PendingTask) (map[string]string, error) {
...
// TODO for SDK
func getTemplateSize(pendingtask *PendingTask) string {
	template := ""
...
func (this *Reservation) RequestPlacement(pending []*PendingTask, requestSpec, filterProperties map[string]string) (map[string]string, error) {
...
func CreateWatcher(client *MesosClient, mesosmetadata *ConnectionClient) {
	for {
		time.Sleep(time.Second * 30)
		pending, err := RequestPendingTasks(client)
		if err != nil {
		...
		if len(pending) > 0 {
			...
			for i := range pending {
				...
				*/
				...
				res, err := RequestMesosAction(client)
				if err != nil {
